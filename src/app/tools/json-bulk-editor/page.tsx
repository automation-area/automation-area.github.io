...
import { parseScalar } from "@/lib/parse-scalar";
...
export default function JsonBulkEditor() {
  ...
  // Update a single cell. Values are typed via parseScalar so "true"/"123"
  // become boolean/number while "007"-style strings stay strings.
  const handleCellChange = (rowIndex: number, colKey: string, val: string) => {
    const newData = [...data];
    newData[rowIndex] = { ...newData[rowIndex], [colKey]: parseScalar(val) };
    setData(newData);
  };
  ...
  const handleBulkUpdate = () => {
    ...
    const parsedVal = parseScalar(bulkVal);
  ...
  const outputText = data.length > 0 ? JSON.stringify(data, null, 2) : "";

  return (
    <ToolLayout
      glow="top-0 right-0 bg-blue-500/10"
      maxWidth="max-w-7xl"
      heading={
        <>
      ...
      description="Paste a JSON array generated from the Dummy Data Factory (or anywhere else) to view it as a spreadsheet. Individually modify cells or bulk replace entire columns with your real data."
    >
      {/* Workspace */}
      <div className="flex flex-col lg:flex-row gap-6 flex-grow min-h-0">
        ...
        <div className="w-full lg:w-1/3 flex flex-col gap-6 shrink-0 h-[800px] lg:h-auto">
          {/* Input Phase */}
          <div className="flex-1 min-h-0 bg-white/5 border border-white/10 rounded-2xl backdrop-blur-md p-5 flex flex-col gap-3">
