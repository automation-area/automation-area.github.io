...
export default function EnvConverter() {
  ...
  const { outputText, errorLine } = useMemo(() => {
    ...
    try {
      ...
      const parseEnv = (text: string) => {
        ...
        for (const line of lines) {
          ...
          if (match) {
            ...
            // .env conventionally uses upper-case booleans too (TRUE/FALSE)
            const lower = val.toLowerCase();
            const parsedVal: unknown =
              lower === "true" ? true : lower === "false" ? false : parseScalar(val);
      ...
      // Helper to flatten JSON into .env lines (keys upper-cased, values untouched)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const flattenObject = (obj: any, prefix = ""): string[] => {
        ...
        for (const key in obj) {
          if (typeof obj[key] === "object" && obj[key] !== null && !Array.isArray(obj[key])) {
            ...
            lines.push(`${(prefix + key).toUpperCase()}=${val}`);
          }
        }
        return lines;
      };
      ...
      let out = "";
      if (outputFormat === "env") {
        out = flattenObject(parsedObj).join("\n");
...
FEATURES_BETA=false`);
  ...
  return (
    <ToolLayout
      glow="top-0 right-1/4 bg-fuchsia-500/10"
      heading={
        <>
        ...
        <div className="flex flex-col h-full bg-white/5 border border-white/10 p-6 rounded-2xl backdrop-blur-md shadow-lg group hover:border-fuchsia-500/30 transition-colors">
          <div className="flex justify-between items-center mb-4">
            ...
            <CopyButton
              text={outputText}
              label="Copy Output"
              idleClass="bg-gradient-to-r from-purple-500 to-fuchsia-500 text-white font-medium hover:from-purple-400 hover:to-fuchsia-400 shadow-lg"
              copiedClass="bg-fuchsia-500/20 text-fuchsia-400 border border-fuchsia-500/50"
            />
          </div>
          <textarea
            readOnly
